...
HELP_SECTIONS: list[tuple[str, str, str]] = [
    ...
    (
        ...
        <ul>
            ...
            <li><b>왼쪽 체크박스</b> 로 여러 종목을 골라 <b>🗑 삭제</b> 로 한 번에 삭제
                (헤더 체크박스는 전체 선택). 두 개 이상 고르면 <b>추가·수정·정렬은
                잠깁니다</b> — 여러 종목에 걸치면 어디에 적용되는지 모호해서예요.</li>
    ...
    (
        ...
        "🏦 여러 계좌로 나눠 관리하기",
        """
        <p>주계좌·연금·ISA 처럼 증권 계좌를 나눠서 종목을 관리할 수 있습니다.
        <b>같은 종목을 계좌마다 다른 평단가·수량으로</b> 따로 들고 있어도 각각 그대로
        표시돼요.</p>
        <p>계좌를 나눠 쓰지 않으면 계좌 관련 UI 는 <b>아무데도 나타나지 않습니다.</b>
        계좌를 하나 더 만드는 순간부터 보이기 시작해요.</p>

        <h3>📂 계좌 만들기 · 고치기</h3>
        <ul>
            <li><b>Windows</b>: 트레이 메뉴 → <b>📈 보유종목 → 🏦 계좌 관리</b></li>
            <li><b>macOS</b>: 팝오버 상단 <b>계좌 줄을 우클릭</b></li>
            <li>계좌마다 <b>이름(6자까지)</b> 과 <b>색</b> 을 정합니다. 이 색이 계좌 선택
                UI 와 종목 표시에 함께 쓰여요.</li>
            <li>▲▼ 로 계좌 순서를 바꿀 수 있습니다.</li>
        </ul>

        <h3>👁 계좌별로 보기</h3>
        <ul>
            <li><b>Windows</b>: 마스터 위젯의 <b>계좌</b> 드롭다운 ·
                <b>macOS</b>: 팝오버 상단 계좌 버튼</li>
            <li><b>전체</b> 또는 <b>계좌 하나</b>를 고릅니다. 여러 계좌를 동시에 켜는
                조합은 없어요 — 요약 숫자가 무엇의 합인지 흐려지기 때문입니다.</li>
            <li>계좌 필터와 <b>시장 필터(전체·한국·미국)</b> 는 함께 걸립니다.</li>
            <li><b>요약(총 매입금액·평가금액·평가손익·수익률)</b> 은 지금 화면에 보이는
                종목만 더한 값입니다.</li>
            <li><b>전체</b> 로 보는 중일 때, <b>Windows</b> 는 위젯 왼쪽에 계좌색 막대가 ·
                <b>macOS</b> 는 종목 행에 계좌 뱃지가 붙어 어느 계좌 것인지 구분됩니다.</li>
        </ul>

        <h3>↔️ 계좌 옮기기</h3>
        <p>종목의 계좌는 <b>수정 창</b>이나 <b>종목 관리 표의 계좌 칸</b>에서 바꿉니다.
        옮겨도 위젯 위치와 메모는 그대로 따라갑니다.</p>
        <p>옮긴 계좌에 <b>같은 종목이 이미 있으면</b> 두 보유분을 하나로 합칠지
        물어봅니다. 합치면 평단가는 수량 기준으로 다시 계산되고
        (예: 10주 70,000원 + 5주 60,000원 → 15주 66,667원), 되돌릴 수 없습니다.</p>

        <h3>🗑 계좌 지우기</h3>
        <p>종목이 남아 있는 계좌를 지우면 그 종목들을 어떻게 할지 물어봅니다 —
        <b>다른 계좌로 옮기기</b> 또는 <b>종목도 함께 삭제</b>. 계좌가 하나뿐일 때는
        지울 수 없어요 (종목을 둘 곳이 없어집니다).</p>

        <p>💡 <b>관심종목은 계좌를 타지 않습니다.</b> 어느 계좌를 보고 있든 하나의
        목록을 그대로 공유해요.</p>
        """,
    ),
    (
        ...
        <h3>📥 보유종목 동기화</h3>
        <ul>
            <li>관리창의 <b>📥 보유종목 동기화</b> 는 현재 보유 중인 종목 가운데
                <b>태그가 없는 것</b>을 <b>'보유중'</b> 태그로 관심종목에 한 번에 추가합니다.</li>
            <li>이미 다른 태그가 달린 종목은 그대로 둡니다(옮기지 않음).</li>
        </ul>
        ...
        <ul>
            <li>관심종목 행의 미니 차트에 <b>마우스를 올리면</b> 일봉 차트가 크게 떠
                <b>5 · 20 · 60일 이동평균선</b>과 함께 보입니다.
                (이미 받아둔 캔들 재사용 — 추가 네트워크 호출 없음)</li>
            <li>관리창의 <b>확대 차트 표시 기간</b> 드롭다운으로 <b>1~6개월</b> 중 고를 수 있습니다(기본 3개월).
                기간이 길수록 차트 가로폭도 함께 넓어집니다.</li>
            <li>이동평균선 표시 여부는 관리창의 <b>확대 차트 이동평균선</b> 체크박스(5·20·60일)로 켜고 끕니다.</li>
            <li>관리창의 <b>확대 차트 보조축</b> 체크박스로 <b>날짜축</b>(아래에 대략적 월/일)과
                <b>가격축</b>(오른쪽에 최고가·평균가·최저가)을 각각 켜고 끌 수 있습니다.</li>
        </ul>
        """,
    ),
    (
    ...
    (
        ...
        <h3>🏦 계좌를 나눠 쓸 때</h3>
        <ul>
            <li><b>첫 번째 시트</b>는 전 계좌를 합친 목록입니다. 계좌 컬럼과 계좌별
                소계가 함께 들어가고, 하단에는 전체 요약이 붙습니다.</li>
            <li><b>두 번째 시트부터는 계좌마다 한 장</b>씩 — 그 계좌의 종목과 요약만
                담깁니다.</li>
            <li>가져올 때는 계좌 시트를 먼저 봅니다. 계좌 <b>이름·색·소속</b>이 그대로
                복원돼요.</li>
            <li>계좌 시트를 지우고 첫 시트만 남겨도, <b>계좌 컬럼</b>만 있으면 계좌를
                다시 만들어 배정합니다. 이 컬럼을 Excel 에서 직접 고쳐 종목을 다른
                계좌로 옮길 수도 있어요.</li>
            <li><b>병합</b>은 같은 계좌의 같은 종목끼리만 맞춰 갱신합니다. 다른 계좌에
                있는 같은 종목은 서로 건드리지 않아요.</li>
            <li><b>덮어쓰기</b>는 계좌 구성까지 Excel 의 것으로 교체합니다.</li>
        </ul>
        <p>계좌를 나눠 쓰지 않으면 시트는 한 장뿐이고 계좌 컬럼도 없습니다 — 계좌 기능
        이전에 만든 파일과 똑같은 모양이라 서로 그대로 주고받을 수 있어요.</p>
        """,
    ),
    (
        "📝  메모장",
        ...
        <h3>🗒 종목별 메모 (종목마다 따로)</h3>
        <p>전역 메모장과 별개로, <b>종목마다</b> 메모를 따로 남길 수 있습니다.</p>
        <ul>
            <li><b>작성</b> — 위젯·팝오버의 <b>종목 행을 우클릭 → 📝 메모</b>. 종목별로
                창이 따로 떠서 여러 종목 메모를 동시에 열어둘 수 있어요.</li>
            <li><b>모아보기</b> — 트레이(메뉴바) 메뉴 → <b>🗒 종목별 메모</b> 에서 메모를 남긴
                종목을 <b>카드 리스트</b>로 한눈에 봅니다. 카드를 클릭하면 그 종목 메모창이 열려요.</li>
            <li><b>자동 갱신</b> — 메모를 추가·수정하거나 종목을 삭제하면 모아보기 목록이
                바로 갱신됩니다. 메모창을 열면 입력칸에 바로 포커스가 가서 이어서 적을 수 있어요.</li>
        </ul>
        """,
    ),
    (
    ...
    (
        ...
        <ul>
            ...
            <li><b>여러 개를 한꺼번에 옮기기</b>: 트레이 →
                <b>📐 화면 정렬 → 🔲 위젯 묶어 옮기기</b> (아래 참고)</li>
        ...
        <h3>🔲 위젯 묶어 옮기기 (Windows)</h3>
        <p>종목이 많아지면 위젯을 하나씩 끌어 옮기기가 번거롭습니다. 영역으로 여러 개를
        묶어 한꺼번에 옮길 수 있어요.</p>
        <ul>
            <li>트레이 → <b>📐 화면 정렬 → 🔲 위젯 묶어 옮기기</b> 를 고르면 커서가
                <b>십자</b>로 바뀝니다.</li>
            <li><b>드래그로 사각형을 그리면</b> 그 영역에 걸친 위젯들이 묶입니다
                (완전히 감싸지 않고 걸치기만 해도 잡혀요). 묶인 위젯은 테두리가
                강조색으로 바뀝니다.</li>
            <li>묶인 위젯 <b>아무거나 끌면</b> 나머지가 <b>서로 간격을 유지한 채</b>
                같이 움직입니다. 옮긴 위치는 바로 저장돼요.</li>
            <li>묶음은 <b>옮긴 뒤에도 유지</b>돼 여러 번 나눠서 위치를 다듬을 수 있습니다.</li>
        </ul>
        <p><b>묶음 해제</b> — 묶음 밖의 다른 위젯 클릭 · <b>Esc</b> · 메뉴 재실행.
        <b>위치 초기화</b>·<b>마스터 화면에 정렬</b>·필터 변경처럼 위젯이 자동으로
        재배치되는 동작에서는 알아서 풀립니다.</p>
        <p>💡 투명도가 <b>50% 이하</b>일 때는 위젯이 클릭을 통과시켜 옮길 수 없어,
        이 기능도 잠깁니다. 슬라이더를 올린 뒤 사용하세요.</p>
        ...
        <ul>
            ...
            <li><b>팝오버 밖 클릭</b>: 자동으로 닫힘 (<b>📌 고정</b> 중이면 유지)</li>
        </ul>

        <h3>🪟 macOS 보유/관심 분리 (별도 창으로 떼어 보기)</h3>
        <ul>
            <li><b>탭 전환</b> — 팝오버 상단 <b>보유 / 관심</b> 탭으로 두 목록을 전환합니다.</li>
            <li><b>분리</b> — <b>보유</b> 또는 <b>관심</b> 탭을 <b>팝오버 밖으로 드래그</b>하면
                그 목록이 <b>독립 창</b>으로 떨어져 나옵니다. 보유 손익과 관심 시세를
                동시에 볼 수 있어요.</li>
            <li><b>합치기</b> — 분리된 창의 <b>⇤ 합치기</b> 버튼을 누르면 다시 팝오버로
                합쳐집니다. (그 버튼으로만 합쳐져요)</li>
            <li><b>창 이동</b> — 팝오버·분리 창 모두 <b>상단 탭 줄의 빈 공간</b>
                (보유는 요약 카드도 가능)을 잡고 끌어 옮깁니다.</li>
            <li><b>함께 토글</b> — 분리한 상태에서도 <b>메뉴바 아이콘 클릭</b>은 두 창을
                <b>함께</b> 표시/숨김합니다. 한 목록만 보고 싶으면 합쳐서 탭으로 보세요.</li>
            <li><b>창별 설정</b> — 분리 창도 <b>📌 고정 · 투명도 · 시장 필터(전체/한국/미국)
                · 위치</b>를 각각 따로 가집니다. <b>📌 고정</b>하면 다른 곳을 클릭해도
                그 창은 사라지지 않아요. 분리 상태와 창 위치는 저장돼 다음 실행에 복원됩니다.</li>
        </ul>
        """,
    ),
]
