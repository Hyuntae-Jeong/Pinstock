...
| **🔲 위젯 묶어 옮기기** | 트레이 → 📐 화면 정렬. 영역을 드래그해 여러 위젯을 묶고 한꺼번에 이동 |
...
종목이 많으면 위젯을 하나씩 끌어 옮기기가 번거롭습니다. **트레이 → 📐 화면 정렬 → 🔲 위젯 묶어 옮기기** 를 고르면 커서가 십자로 바뀌고, 드래그한 사각형에 **걸친** 위젯들이 묶입니다. 묶인 위젯 아무거나 끌면 나머지가 **간격을 유지한 채** 함께 움직이고, 위치는 바로 저장됩니다.

묶음은 옮긴 뒤에도 유지돼 여러 번 나눠 다듬을 수 있습니다. 해제는 **묶음 밖 다른 위젯 클릭 · Esc · 메뉴 재실행** 이며, 위치 초기화·마스터 화면에 정렬·필터 변경처럼 자동 재배치가 도는 동작에서는 알아서 풀립니다. 투명도가 50% 이하일 때는 위젯이 클릭을 통과시켜 옮길 수 없어 이 기능도 잠깁니다.
...
### 📤 Excel 내보내기 · 가져오기 (계좌 포함)

- **첫 번째 시트** = 전 계좌 합산 목록. 계좌를 나눠 쓰면 **계좌 컬럼**과 **계좌별 소계**가 함께 들어갑니다.
- **두 번째 시트부터는 계좌마다 한 장**씩 — 그 계좌의 종목과 요약만 담깁니다.
- 가져올 때는 계좌 시트를 먼저 봅니다. 계좌 **이름·색·소속**이 그대로 복원됩니다.
- 계좌 시트를 지워도 첫 시트의 **계좌 컬럼**만 있으면 계좌를 다시 만들어 배정합니다. 이 컬럼을 Excel 에서 직접 고쳐 종목을 다른 계좌로 옮길 수도 있습니다.
- **병합**은 `(계좌, 종목코드)` 가 같은 것끼리만 갱신합니다. 다른 계좌의 같은 종목은 서로 건드리지 않습니다. **덮어쓰기**는 계좌 구성까지 Excel 의 것으로 교체합니다.
- 계좌를 나눠 쓰지 않으면 시트는 한 장뿐이고 계좌 컬럼도 없습니다 — 계좌 기능 이전 파일과 같은 모양입니다.
