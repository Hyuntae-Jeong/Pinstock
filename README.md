...
> 가상환경을 안 만들 경우 `python -m pinstock` 대신 본인의 파이썬 경로를 직접 써도 됩니다.
...
| **시스템 트레이 우클릭** | 종목 추가 / 관리 / Excel 내보내기·가져오기 / 위치 초기화 / 종료 |
...
pyinstaller --onefile --windowed --name "주식위젯" pinstock/__main__.py
```
생성된 `dist/주식위젯.exe` 사용.

### macOS (`.app`)
```bash
pip install pyinstaller
pyinstaller --onefile --windowed --name "Pinstock" pinstock/__main__.py
```
생성된 `dist/Pinstock.app` 사용.

> macOS 는 코드 서명을 하지 않은 경우 "확인되지 않은 개발자" 경고가 뜹니다. Finder 에서 우클릭 → "열기" 로 한 번 허용하면 이후엔 그냥 실행됩니다.
